...
export function FuzzyFinder() {
  ...
  const files = activeProject?.fileIndex.files ?? [];
  const isLoading = activeProject?.fileIndex.isLoading ?? false;
  const lastIndexed = activeProject?.fileIndex.lastIndexed ?? 0;
